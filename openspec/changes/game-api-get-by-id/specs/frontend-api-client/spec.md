...
The frontend SHALL use a typed `GameApiClient` service that communicates with the GameService-backed public API. Return types SHALL reflect the `202 Accepted` response pattern for create and move operations while preserving typed response models for read operations.
...
- **THEN** it SHALL send `GET /api/games/{gameId}` to the GameService public API
