...
public sealed record GameState(Models.GameState Game, int Row, int Col) : IRequest<GameLogicMoveResult>;
...
public sealed class GameStateHandler(
    IRequestHandler<CheckWinner, CheckWinnerResult> checkWinnerHandler,
    IRequestHandler<CheckDraw, CheckDrawResult> checkDrawHandler)
    : IRequestHandler<GameState, GameLogicMoveResult>
{
    public async Task<GameLogicMoveResult> HandleAsync(GameState request, CancellationToken ct = default)
    {
        ...
        var winnerResult = await checkWinnerHandler.HandleAsync(new CheckWinner(request.Game.Board), ct);
        ...
        var drawResult = await checkDrawHandler.HandleAsync(new CheckDraw(request.Game.Board), ct);
