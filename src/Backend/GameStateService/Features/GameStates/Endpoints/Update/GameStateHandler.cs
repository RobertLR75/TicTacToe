...
public sealed record ApplyMove(GameEntity Game, int Row, int Col) : IRequest<GameLogicMoveResult>;
...
public sealed class GameStateHandler(
    ...
    : IRequestHandler<ApplyMove, GameLogicMoveResult>
{
    public async Task<GameLogicMoveResult> HandleAsync(ApplyMove request, CancellationToken ct = default)
    {
        ...
        var winnerResult = checkStateService.CheckWinner(request.Game.Board);
        ...
        var drawResult = checkStateService.CheckDraw(request.Game.Board);
